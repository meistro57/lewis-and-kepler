...
**2. Magnetism / precession / coherence.** A conventional magnetic-dipole model (MAGNETO-GEOMETRY-009) admits a locally-stable cubic candidate AND a lower-energy ring; neither is established as global ground state.

**3. MAGNETIC-RESPONSE-011 (verified by Lewis, 8.9e-15).** Under a weak uniform field the cube keeps its geometry *and* develops a nonzero net moment along the exact linear branch `M_parallel/(8m) = χ·b`, `χ = 4/(A − D²/C) = 0.2903010825318187` → **8.709% of full alignment at b=0.30**. The coefficient identity `E/E0 = U_c + A q² + C w² + 2D q w − 8bq` was reproduced independently (158k random orientations, max error 1.07e-14). This **resolves alignment-vs-precession for this model: alignment wins** — the moments tilt smoothly along the field, no circulating family-drift. It also **corrects an earlier Lewis overclaim**: the point set keeps full cubic symmetry while the *magnetic pattern* breaks it (G24→4 proper rotations); magnetism does not require the geometry to break symmetry first.

**Still open (the packet's own boundary):** no global ground state (ring still lower than cube at zero field); no hysteresis/memory claim (a changed zero-field pattern is not yet a tested memory device); computational quasistatic SLSQP, not physical time/damping — the low-field "jumps" moved with step size and are not claimed as switching thresholds. Next test the report itself names: add rotational/translational damping and replay one X + one diagonal cycle at several rates. See `findings/provisional-entries-keystone-coherence.md`, Kepler's `findings/motion-vs-rotational-dynamics.md`, and the open `Coherence = Coupling × Orientation × Coordination` invariant.
